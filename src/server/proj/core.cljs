(ns proj.core
  (:require [clojure.browser.repl :as repl]
            [server.utilities]
            [server.AIagent.combination :as combination]
            [server.AIagent.knobbification :as knobbification]
            [server.AIagent.analogy :as analogy]
            [server.AIagent.self-watching :as selfwatch]
            [server.AIagent.subsumption :as subsumption]
            )
  )
...
(defn ^:export combinationalAI [scoreEvents]
  (combination/combinational-parameters scoreEvents)
  )

(defn ^:export knobbificationAI [scoreEvents]
  (knobbification/knobbification scoreEvents)
  )

(defn ^:export analogyAI [scoreEvents]
  (analogy/analogy-system scoreEvents)
  )

(defn ^:export selfwatchingAI [scoreEvents]
  (selfwatch/self-watching-system scoreEvents)
  )
